...
def update_edges(a, b, num, player): 
    if board[a][b][num] == 0:
        board[a][b][num] = player

def update_sq(a, b, p1, p2, player, sq_t):
...
def select_edge(mouse_x, mouse_y, p1, p2, player, sq_t):
    ...
    # vertical edges
        ...
        if 0 < round_x < DIM: # overlapping left/right edges
            update_edges(round_x, floor_y, 0, player)
            p1, p2, player, sq_t = update_sq(round_x, floor_y, p1, p2, player, sq_t)
            update_edges(round_x - 1, floor_y, 3, player)
            p1, p2, player, sq_t = update_sq(round_x - 1, floor_y, p1, p2, player, sq_t)
    ...
    # horizontal edges
        ...
        if 0 < round_y < DIM: # overlapping upper/lower edges
            update_edges(floor_x, round_y, 1, player)
            p1, p2, player, sq_t = update_sq(floor_x, round_y, p1, p2, player, sq_t)
            update_edges(floor_x, round_y - 1, 2, player)
            p1, p2, player, sq_t = update_sq(floor_x, round_y - 1, p1, p2, player, sq_t)
...
def draw_score(p1, p2, player, screen):
    ...
    if game_status(p1, p2) != False:
        ...
        text["win"].display_txt(pygame.font.SysFont("freesansbold.ttf", 50), game_status(p1, p2), (0, 184, 148), (WINDOW_X/3, 30))
